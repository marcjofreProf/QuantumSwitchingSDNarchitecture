...
#
# The physical node IP is the one value that varies between deployments and
# has no sensible automatic default. It is prompted once, persisted to
# ~/.quantum-sdn/config.env, and reused on subsequent runs. Scripts and
# services that need it (register-devices.sh, the RESTCONF gateway) read it
# from the environment or from this file.
#
# The 172.21.x.x address family matches the MANO / 6G-OpenLab convention
# used by bootstrap-node.sh, so the two scripts are consistent out of the
# box:
#   - controller / cluster node:  172.21.2.23
#   - quantum node (BeagleBone):  172.21.128.254
#
# Override non-interactively with, for example:
#   QUANTUM_NODE_IP=192.168.1.42 ./bootstrap-quantum-switching-sdn.sh
# ---------------------------------------------------------------------------
...
if [ -t 0 ]; then
    ...
    echo -e "${CYAN}===========================================================${NC}"
    echo -e "${YELLOW}Press ENTER to accept the value in [brackets].${NC}"
    echo

    CONTROLLER_HOST=$(prompt_with_default "Enter the IP address of the Network Controller (this host)" "$CONTROLLER_HOST")
...
# Generated by bootstrap-quantum-switching-sdn.sh — safe to delete.
...
deploy_cloud_native_uonos() {
    ...
    # Push the resolved node identity into the deployment so gateway.py
    # builds KNOWN_DEVICES from the actual values rather than its defaults.
    log_info "Configuring gateway with node identity ($QUANTUM_NODE_ID / $QUANTUM_NODE_IP)..."
    kubectl -n micro-onos set env deployment/restconf-gateway \
        QUANTUM_NODE_ID="$QUANTUM_NODE_ID" \
        QUANTUM_NODE_IP="$QUANTUM_NODE_IP" \
        >/dev/null
